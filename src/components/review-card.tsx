import StarRating from "@/components/star-rating";
import { getInitials, getAvatarColor } from "@/lib/utils";

interface ReviewCardProps {
  reviewerName: string;
  starRating: number;
  reviewText: string;
  profilePicUrl: string | null;
  presetColor?: string;
}

export default function ReviewCard({
  reviewerName,
  starRating,
  reviewText,
  profilePicUrl,
  presetColor,
}: ReviewCardProps) {
  const initials = getInitials(reviewerName || "A");
  const avatarColor = presetColor ?? getAvatarColor(reviewerName || "A");

  return (
    <div
      style={{ width: 480, fontFamily: "Arial, Helvetica, sans-serif" }}
      className="rounded-xl bg-white p-6 shadow-lg"
    >
      {/* Header: avatar + name + stars */}
      <div className="flex items-start gap-3">
        {/* Profile pic */}
        <div
          className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full"
          style={{ backgroundColor: profilePicUrl ? undefined : avatarColor }}
        >
          {profilePicUrl ? (
            <img
              src={profilePicUrl}
              alt={reviewerName}
              className="h-full w-full object-cover"
              crossOrigin="anonymous"
            />
          ) : (
            <span className="text-sm font-semibold text-white">{initials}</span>
          )}
        </div>

        <div className="flex-1">
          <p className="text-sm font-semibold text-gray-900">
            {reviewerName || "Reviewer Name"}
          </p>
          <div className="mt-0.5">
            <StarRating rating={starRating} size={16} />
          </div>
        </div>
      </div>

      {/* Review text */}
      <p className={`mt-3 text-sm leading-relaxed ${reviewText ? "text-gray-700" : "italic text-gray-400"}`}>
        {reviewText || "terrible service, and the chicken was average"}
      </p>

      {/* Google branding */}
      <div className="mt-4 flex items-center justify-end gap-1.5">
        <svg viewBox="0 0 24 24" width="16" height="16">
          <path
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
            fill="#4285F4"
          />
          <path
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            fill="#34A853"
          />
          <path
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            fill="#FBBC04"
          />
          <path
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            fill="#EA4335"
          />
        </svg>
        <span className="text-xs text-gray-400">Google</span>
      </div>
    </div>
  );
}
